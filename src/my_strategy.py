...
def get_portfolio_bollinger_bands(port_val, daily_rf=0, samples_per_year=252):
    ...
    sma100 =pd.rolling_mean(df, 50)
    sma200 =pd.rolling_mean(df, 100)
    std_sma = pd.rolling_std(df,20)
    upper_band= sma+ 2* std_sma
    lower_band= sma- 2* std_sma

    return sma, sma100, sma200, upper_band, lower_band
"""
...
def orderbook_generator(events, symbol, sma50, sma100, stock_shares=100 ):
    df= events.copy()
    ...
    for i in range(0,len(df.index)):
        if df['events'][i] =='longentry':
            if (flag_long == 0) and (flag_short ==0) and (sma50[i] > sma100[i]):
                flag_long = 1
        ...
        if df['events'][i] =='shortentry':
            if flag_short ==0 and (flag_long ==0) and (sma50[i]< sma100[i]):
                flag_short =1
                order_book = order_book.append({'Date':df.index[i], 'Symbol':symbol, 'Order':'SELL','Shares':stock_shares},ignore_index=True)
                updated_events = updated_events.append({'Date': df.index[i], 'events':'shortentry'},ignore_index=True)
            #if flag_short ==0 and (flag_long ==0) and (sma50[i]> sma100[i]):
...
def assess_portfolio(start_date, end_date, symbols, allocs, number_of_stocks=1):
    ...
    sma, sma50, sma100, upper_band, lower_band = get_portfolio_bollinger_bands(port_val)

    #cum_ret_SPY, avg_daily_ret_SPY, std_daily_ret_SPY, sharpe_ratio_SPY = get_portfolio_stats(prices_SPY)
    sma_SPY, sma100_SPY, sma200_SPY, upper_band_index_SPY, lower_band_SPY = get_portfolio_bollinger_bands(prices_SPY)
    #plot_bollinger_band_data_direct (pd.concat([prices, sma, sma50, sma100, upper_band, lower_band, sma_SPY], axis=1))
    events = detect_events(port_val, sma, upper_band, lower_band)

    # Compare daily portfolio value with SPY using a normalized plot
    df_bband = pd.concat([prices, sma, upper_band, lower_band, sma_SPY], axis=1)
    df_bband.columns =['Price','Sma', 'Upperband', 'Lowerband', 'SMA_SPY']
    ...
    updated_events = orderbook_generator(df_events, ''.join(symbols),sma, sma_SPY, number_of_stocks)
...
def plot_bollinger_band_data(df_bband, symbol, df_events, title="Stock prices", xlabel="Date", ylabel="Price"):
    ...
    df['SMA_SPY'].plot(label='SMA_SPY', ax=ax,color='purple', legend=True)
    #df['SMA100'].plot(label='SMA100', ax=ax,color='blue', legend=True)
    ...
    plt.vlines(longentry.index, plt.ylim()[0], plt.ylim()[1] ,color ='g')
    ...
    fig=plt.gcf()
    fig.savefig('Bollinger band.png')
###############################
...
def test_run():
    ...
    prices_index= get_data(['SPY'], pd.date_range(start_date, end_date))
    prices_index = prices_index[['SPY']]  # remove SPY
    #print prices_index
